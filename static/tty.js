...
tty.open = function() {
  ...
  h1 = tty.elements.h1;
  open1 = tty.elements.open1;
  open2 = tty.elements.open2;
  open3 = tty.elements.open3;
  open4 = tty.elements.open4;
  input1 = tty.elements.input1;
  input2 = tty.elements.input2;
  input3 = tty.elements.input3;
  input4 = tty.elements.input4;
  lights = tty.elements.lights;

  if (open1) {
    on(open1, 'click', function() {
      if(tty.opened['1'] === true) return;
      new Window(input1.value);
      tty.opened['1'] = true;
    });
  }
  ...
  if (open2) {
    on(open2, 'click', function() {
      if(tty.opened['2'] === true) return;
      new Window(input2.value);
      tty.opended['2'] = true;
    });
  }

  if (open3) {
    on(open3, 'click', function() {
      if(tty.opened['3'] === true) return;
      new Window(input3.value);
      tty.opened['3'] = true;
    });
  }

  if (open4) {
    on(open4, 'click', function() {
      if(tty.opened['4'] === true) return;
      new Window(input4.value);
      tty.opened['4'] = true;
    });
  }
...
function Window(containerID) {
  ...
  // By default, we will create a tab
  this.createTab();
  this.focus();
  this.bind();
...
function Tab(win, socket) {
  ...
  this.socket.emit('create', cols, rows, 'docker', ['exec', '-it', win.containerID, 'bash'], function(err, data) {
